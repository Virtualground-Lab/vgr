    ...
    FinalizableCrowdsale
{
    ...
    function createContinuousSaleContract()
        internal
        returns (VGRContinuousSale)
    {
        return new VGRContinuousSale(rate, wallet, token);
    }
    ...
    function setInitialRate(uint256 rate) public onlyOwner {
        require(rate != 0);
        require(block.number < startBlock);
    ...
    function setEndRate(uint256 rate) public onlyOwner {
        require(rate != 0);
        require(block.number < startBlock);
    ...
    function beginContinuousSale() public onlyOwner {
        require(isFinalized);
